	...
	return codegen.NewJenniferSection("jsonrpc-client-struct", func(stmt *jen.Statement) {
		...
		stmt.Type().Id(data.ClientStruct).StructFunc(func(g *jen.Group) {
			...
			if httpcodegen.HasWebSocket(data) {
				...
				g.Line()
				g.Id("connMu").Qual("sync", "Mutex")
				g.Comment("conn is the connection shared by the client and its streams. It reads")
				g.Comment("the connection once and routes every response to its stream.")
				g.Id("conn").Op("*").Qual("github.com/CaliLuke/loom/jsonrpc", "WebSocketClientConn")
