	...
	return codegen.NewJenniferSection("jsonrpc-client-struct", func(stmt *jen.Statement) {
		...
		stmt.Type().Id(data.ClientStruct).StructFunc(func(g *jen.Group) {
			...
			if httpcodegen.HasWebSocket(data) {
				...
				g.Comment("conn is the connection shared by the client and its streams. Both close")
				g.Comment("it through this one wrapper, so it is closed only once.")
				g.Id("conn").Op("*").Add(codegen.TypeRef("loomhttp.WebSocketStream"))
