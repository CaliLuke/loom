...
func renderJSONRPCServerHandlerInit(e *httpcodegen.EndpointData) string {
	...
	fmt.Fprintf(&b, "\t\tctx = context.WithValue(ctx, goa.MethodKey, %q)\n", e.Method.Name)
	fmt.Fprintf(&b, "\t\tctx = context.WithValue(ctx, goa.ServiceKey, %q)\n\n", e.ServiceName)
...
func renderSSEHandlerInitBody(b *strings.Builder, e *httpcodegen.EndpointData) {
	...
	b.WriteString("\t\t\t\t\t\treturn strm.sendError(ctx, jsonrpc.IDToString(req.ID), jsonrpc.MethodNotFound, goa.ErrorSafeMessage(err), jsonrpc.NewErrorData(err))\n")
	b.WriteString("\t\t\t\t\t}\n")
	b.WriteString("\t\t\t\t}\n")
	...
	b.WriteString("\t\t\t\t}\n")
	b.WriteString("\t\t\t\treturn strm.sendError(ctx, jsonrpc.IDToString(req.ID), code, goa.ErrorSafeMessage(err), jsonrpc.NewErrorData(err))\n")
	b.WriteString("\t\t\t}\n")
	b.WriteString("\t\t\treturn nil\n")
	b.WriteString("\t\t}\n")
	b.WriteString("\t\treturn nil\n")
...
func renderJSONRPCStandardHandlerInitBody(b *strings.Builder, e *httpcodegen.EndpointData) {
	if e.Payload != nil && e.Payload.Ref != "" {
		...
		if httpcodegen.IsWebSocketEndpoint(e) {
			...
			b.WriteString("\t\t\t\t\tcode = jsonrpc.InvalidParams\n")
			b.WriteString("\t\t\t\t}\n")
			b.WriteString("\t\t\t\tencodeJSONRPCError(ctx, w, req, code, goa.ErrorSafeMessage(err), jsonrpc.NewErrorData(err), encoder, errhandler)\n")
			b.WriteString("\t\t\t} else {\n")
	...
	for _, gerr := range e.Errors {
		for _, item := range gerr.Errors {
			...
			fmt.Fprintf(b, "\t\t\t\tcase %q:\n", item.Name)
			fmt.Fprintf(b, "\t\t\t\t\tencodeJSONRPCError(ctx, w, req, %d, goa.ErrorSafeMessage(err), jsonrpc.NewErrorData(err), encoder, errhandler)\n", item.Response.Code)
		}
	}
	b.WriteString("\t\t\t\tcase \"invalid_params\":\n")
	b.WriteString("\t\t\t\t\tencodeJSONRPCError(ctx, w, req, jsonrpc.InvalidParams, goa.ErrorSafeMessage(err), jsonrpc.NewErrorData(err), encoder, errhandler)\n")
	b.WriteString("\t\t\t\tcase \"method_not_found\":\n")
	b.WriteString("\t\t\t\t\tencodeJSONRPCError(ctx, w, req, jsonrpc.MethodNotFound, goa.ErrorSafeMessage(err), jsonrpc.NewErrorData(err), encoder, errhandler)\n")
	b.WriteString("\t\t\t\tdefault:\n")
	b.WriteString("\t\t\t\t\tcode := jsonrpc.InternalError\n")
	b.WriteString("\t\t\t\t\tif _, ok := err.(*goa.ServiceError); ok {\n")
	b.WriteString("\t\t\t\t\t\tcode = jsonrpc.InvalidParams\n")
