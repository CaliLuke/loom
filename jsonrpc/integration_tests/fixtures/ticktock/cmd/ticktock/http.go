...
func handleHTTPServer(ctx context.Context, u *url.URL, clockEndpoints *clock.Endpoints, clockSvc clock.Service, wg *sync.WaitGroup, errc chan error, dbg bool) {
	...
	// responses.
	var (
		clockJSONRPCServer *clockjssvr.Server
	)
	{
		eh := errorHandler(ctx)
