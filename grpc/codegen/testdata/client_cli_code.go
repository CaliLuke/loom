...
func BuildMethodAPayload(payloadWithValidationMethodAMetadataInt string, payloadWithValidationMethodAMetadataString string) (*payloadwithvalidation.MethodAPayload, error) {
	...
	var metadataInt *int
	{
		if payloadWithValidationMethodAMetadataInt != "" {
			...
			if *metadataInt < 0 {
				err = loom.MergeErrors(err, loom.InvalidRangeError("MetadataInt", *metadataInt, 0, true))
			}
			if *metadataInt > 100 {
				err = loom.MergeErrors(err, loom.InvalidRangeError("MetadataInt", *metadataInt, 100, false))
			}
			if err != nil {
				return nil, err
			}
