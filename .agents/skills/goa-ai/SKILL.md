---
name: goa-ai
description: Build and maintain Goa-AI services in Go. Use this skill when a user mentions Goa-AI, Goa-AI DSL, `goa gen`, generated `gen/` transport code, OpenAPI/proto generation, service implementation after DSL changes, or refactoring a project with a `design` package.
---
# Goa-AI

Use this skill for Goa-AI work only.
...
- SSE server streams do not expose a generated `Open()` hook. Goa writes SSE headers on the first `Send`, so idle subscriptions that must complete the HTTP handshake before the first business event need a non-generated transport/runtime flush path or an explicit bootstrap event in the contract.
...
- For responses that need multiple `Set-Cookie` headers, prefer idiomatic Goa cookies in the DSL. If a flow still depends on raw cookie header strings, write them from non-generated transport code against the live `http.ResponseWriter` rather than patching generated encoders.
...
- `references/user-guides/quickstart/first-service.md`: create the project, define your agent DSL, and generate Goa-AI code
...
- `references/user-guides/dsl-reference.md`: complete Goa-AI DSL reference (agents, toolsets, policies, MCP)
...
- `references/user-guides/production.md`: production deployment patterns for Goa-AI (Temporal, rate limiting, stream profiles, reminders)

### Code Generation

- `references/user-guides/codegen/commands-and-workflow.md`: `goa gen`, `goa example`, and normal regeneration flow
...
- `references/user-guides/interceptors/goa-interceptors.md`: generated wrapper model, accessor contracts, ordering
...
Use this when you need to jump directly to a bounded chunk of Goa-AI content without browsing the full transcript tree.
