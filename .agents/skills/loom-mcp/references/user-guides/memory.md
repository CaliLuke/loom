...
- A tool-using assistant message typically looks like: ThinkingPart (when present), then optional TextPart, then one or more ToolUseParts
- A user/tool result message typically contains one or more ToolResultParts referencing previous tool_use IDs, plus optional user content (TextPart, ImagePart, DocumentPart)
...
- Planners: Receive the current transcript in planner.PlanInput.Messages and planner.PlanResumeInput.Messages. Can decide what to do based based on the messages, without threading extra state.
...
--- CODE ---
import "github.com/CaliLuke/loom-mcp/runtime/agent/transcript"
...
--- CODE ---
import (
    memorymongo "github.com/CaliLuke/loom-mcp/features/memory/mongo"
    memorymongoclient "github.com/CaliLuke/loom-mcp/features/memory/mongo/clients/mongo"
    runlogmongo "github.com/CaliLuke/loom-mcp/features/runlog/mongo"
    runlogmongoclient "github.com/CaliLuke/loom-mcp/features/runlog/mongo/clients/mongo"
...
- Always correlate tool results: make sure tool implementations and planners preserve tool_use IDs and map tool results back to the correct ToolResultPart via ToolResultPart.ToolUseID
