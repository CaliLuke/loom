...
This guide demonstrates how to compose agents by treating one agent as a tool of another, and explains how Goa-AI models execution as a tree with streaming projections for different audiences.
...
2. Execution-time (tool providers / services) when a provider returns validation errors for bound service methods, include structured field issues in the tool result error so consumers can build the same RetryHint deterministically.
