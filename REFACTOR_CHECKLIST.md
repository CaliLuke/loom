...
- 2026-03-22: Added direct CLI seam coverage in `cmd/loom/generate_test.go` and `codegen/cli/cli_test.go`, lifting `cmd/loom` to 36.3% and `codegen/cli` to 28.0%, then re-ran `make lint` and `LOOM_REPO=/Users/luca/.codex/worktrees/c883/loom make test`.
- 2026-03-22: Replaced an invalid all-in-one DSL transport smoke test with valid focused specimens in `dsl/transport_smoke_test.go` and added JSON-RPC response/config coverage in `jsonrpc/types_config_test.go`, lifting `dsl` to 32.7% and `jsonrpc` to 89.1%, then re-ran `make lint` and `LOOM_REPO=/Users/luca/.codex/worktrees/c883/loom make test`.
