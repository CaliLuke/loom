...
func TestAnalyzeReportsDuplicateAndDesignConflictingRoutes(t *testing.T) {
	root := t.TempDir()
	writeMountHandlerTestModule(t, root, `func routes() {
	mux := loomhttp.NewMuxer()
	mux.Handle("GET", "/duplicate", nil)
	mux.Handle("GET", "/duplicate", nil)
	mux.Handle("POST", "/designed", nil)
	method := "POST"
	mux.Handle(method, "/designed", nil)
	//loom:vet ignore duplicate-route-registration -- exercised by another router in production
	mux.Handle("GET", "/suppressed", nil)
	//loom:vet ignore duplicate-route-registration -- exercised by another router in production
	mux.Handle("GET", "/suppressed", nil)
}
`)
...
`)
	...
	require.NoError(t, err)
	duplicates := diagnosticsForRule(report.Diagnostics, RuleDuplicateRouteRegistration)
	require.Len(t, duplicates, 2)
	require.NotEqual(t, duplicates[0].Location.Line, duplicates[1].Location.Line)
	for _, diagnostic := range duplicates {
		require.Contains(t, diagnostic.Message, "GET /duplicate")
		require.Contains(t, diagnostic.Message, "router.go")
	}
	conflicts := diagnosticsForRule(report.Diagnostics, RuleRouteConflictWithDesign)
	require.Len(t, conflicts, 1)
	require.Contains(t, conflicts[0].Message, "POST /designed")
	require.Contains(t, conflicts[0].Message, "widgets.create")
	require.Equal(t, "router.go", conflicts[0].Location.Path)

	for _, format := range []Format{FormatText, FormatJSON, FormatSARIF} {
		var output bytes.Buffer
		require.NoError(t, WriteReport(&output, report, format))
		require.Contains(t, output.String(), "duplicate-route-registration")
		require.Contains(t, output.String(), "route-conflict-with-design")
	}
}

func TestAnalyzeRouteConflictSuppression(t *testing.T) {
	root := t.TempDir()
	writeMountHandlerTestModule(t, root, `func routes() {
	mux := loomhttp.NewMuxer()
	//loom:vet ignore route-conflict-with-design -- deliberate shadow probe
	mux.Handle("POST", "/designed", nil)
}
`)
	design := designRootWithHTTPRoute("create", "POST", "/designed")
	...
	require.NoError(t, err)
	require.Empty(t, diagnosticsForRule(report.Diagnostics, RuleRouteConflictWithDesign))
	require.Len(t, diagnosticsForRule(report.Diagnostics, RuleRouteOutsideDesign), 1)
}
...
func TestAnalyzeModuleFindsTypedMuxMountHandler(t *testing.T) {
	tests := []struct {
		...
		{
			name: "literal method and path through aliased import",
			source: `func routes() {
	mux := loomhttp.NewMuxer()
	loomhttp.MountHandler(mux, "POST", "/mounted", nil)
}
`,
			want:       "route POST /mounted is registered directly on a Loom mux; declare and mount it through the design",
			wantLength: 1,
		},
		{
			name: "standard library method constant",
			source: `func routes() {
	mux := loomhttp.NewMuxer()
	loomhttp.MountHandler(mux, http.MethodHead, "/mounted-head", nil)
}
`,
			want:       "route HEAD /mounted-head is registered directly on a Loom mux; declare and mount it through the design",
			wantLength: 1,
		},
		{
			name: "dynamic method and path",
			source: `func routes() {
	mux := loomhttp.NewMuxer()
	method := strings.ToUpper("PATCH")
	path := "/dynamic"
	loomhttp.MountHandler(mux, method, path, nil)
}
`,
			want:       "route is registered directly on a Loom mux; declare and mount it through the design",
			wantLength: 1,
		},
		{
			name: "generated file",
			source: `// Code generated by Loom, DO NOT EDIT.
func routes() {
	loomhttp.MountHandler(loomhttp.NewMuxer(), "GET", "/generated", nil)
}
`,
			wantLength: 0,
		},
		{
			name: "source suppression",
			source: `func routes() {
	mux := loomhttp.NewMuxer()
	//loom:vet ignore route-outside-design -- intentional probe
	loomhttp.MountHandler(mux, "OPTIONS", "/probe", nil)
}
`,
			wantLength: 0,
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			root := t.TempDir()
			writeMountHandlerTestModule(t, root, test.source)

			diagnostics, err := analyzeModule(root)

			require.NoError(t, err)
			require.Len(t, diagnostics, test.wantLength)
			if test.wantLength == 1 {
				require.Equal(t, test.want, diagnostics[0].Message)
			}
		})
	}
}
...
func diagnosticsForRule(diagnostics []Diagnostic, rule string) []Diagnostic {
	matching := make([]Diagnostic, 0, len(diagnostics))
	for _, diagnostic := range diagnostics {
...
func designRootWithHTTPRoute(methodName, method, path string) *expr.RootExpr {
	...
	endpoint := &expr.HTTPEndpointExpr{MethodExpr: methodExpr, Service: httpService}
	endpoint.Routes = []*expr.RouteExpr{{Method: method, Path: path, Endpoint: endpoint}}
	httpService.HTTPEndpoints = []*expr.HTTPEndpointExpr{endpoint}
	httpRoot.Services = []*expr.HTTPServiceExpr{httpService}
	return &expr.RootExpr{
		API:      &expr.APIExpr{HTTP: httpRoot},
		Services: []*expr.ServiceExpr{service},
	}
}
...
func writeMountHandlerTestModule(t *testing.T, root, source string) {
	t.Helper()
	writeTestFile(t, filepath.Join(root, "go.mod"), "module example.com/service\n\ngo 1.27.0\n\nrequire github.com/CaliLuke/loom v1.2.3\nreplace github.com/CaliLuke/loom => ./.loomstub\n")
	writeTestFile(t, filepath.Join(root, ".loomstub", "go.mod"), "module github.com/CaliLuke/loom\n\ngo 1.27.0\n")
	writeTestFile(t, filepath.Join(root, ".loomstub", "http", "http.go"), `package http
...
func NewMuxer() Muxer { return &mux{} }

func MountHandler(mux Muxer, method, pattern string, handler http.Handler) {
	mux.Handle(method, pattern, handler.ServeHTTP)
}
`)
	writeTestFile(t, filepath.Join(root, "router.go"), `package service
