...
// fakeServerMain is a server that behaves like the fixture server at startup:
// it logs that it listens before it binds its port. It exits instead of
// binding when it is built with exitAtStart set, as the fixture server does
...
func writeFakeServer(t *testing.T, exitAtStart bool) string {
	t.Helper()
	workDir := t.TempDir()
	mainDir := filepath.Join(workDir, "cmd", "ticktock")
