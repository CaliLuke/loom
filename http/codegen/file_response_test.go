...
func TestFileResponseHTTPSections(t *testing.T) {
	...
	require.Contains(t, handler, `lifecycle.ServeFile(r, o.File, "application/pdf"`)
	require.NotContains(t, handler, "io.Copy(w, o.File")
	metadata := strings.Index(handler, "encodeResponse(ctx, w, o.Result)")
	serve := strings.Index(handler, "lifecycle.ServeFile")
	require.NotEqual(t, -1, metadata)
	require.Less(t, serve, metadata)
	require.NotContains(t, handler, "o.File.Content")
