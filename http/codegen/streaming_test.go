...
func TestWebSocketStreamsHonorContextInGeneratedMethods(t *testing.T) {
	...
	require.Contains(t, recv, "s.conn.ReadJSON(ctx,")
	require.NotContains(t, recv, "context.AfterFunc")
	...
	require.Contains(t, send, "s.conn.WriteJSON(ctx,")
	require.NotContains(t, send, "context.AfterFunc")
	...
	require.Contains(t, clientRecv, "s.conn.ReadJSON(context.Background(),")
	require.Contains(t, clientRecv, "s.conn.ReadJSON(ctx,")
	require.NotContains(t, clientRecv, "context.AfterFunc")
	...
	require.Contains(t, clientSend, "s.conn.WriteJSON(ctx,")
	require.NotContains(t, clientSend, "context.AfterFunc")
