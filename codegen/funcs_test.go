...
func TestSnakeCase(t *testing.T) {
	cases := map[string]struct {
		...
		"latin accents":         {"ÉtéFoo", "été_foo"},
		"latin accent mid":      {"CaféBar", "café_bar"},
		"latin acronym suffix":  {"ÜberAPI", "über_api"},
		"acronym then unicode":  {"HTTPÜber", "http_über"},
		"unicode acronym":       {"ÜBERHttp", "über_http"},
		"acronym before accent": {"IDÉtat", "id_état"},
		"greek":                 {"ΣίσυφοςTest", "σίσυφος_test"},
		"cyrillic":              {"ПриветМир", "привет_мир"},
		"cyrillic digits":       {"Файл2Go", "файл2_go"},
		"title case start":      {"ǅemal", "ǆemal"},
		"title case mid":        {"fooǅemal", "foo_ǆemal"},
		"combining mark":        {"Cafe\u0301Bar", "cafe\u0301_bar"},
		"combining mark upper":  {"E\u0301TE", "e\u0301te"},
		"combining mark word":   {"fooE\u0301t", "foo_e\u0301t"},
		"combining lookahead":   {"ABE\u0301t", "ab_e\u0301t"},
		"precomposed lookahead": {"ABÉt", "ab_ét"},
		"leading combining":     {"\u0301a", "\u0301a"},
		"cjk":                   {"日本語", "日本語"},
		"upper without lower":   {"ϒϒA", "ϒϒ_a"},
		"cjk between words":     {"Foo日本Bar", "foo日本_bar"},
		"cjk then acronym":      {"日本API", "日本_api"},
		"unicode spaces":        {"été\u00a0Foo", "été_foo"},
		"unicode dashes":        {"été-Foo/Bar", "été_foo_bar"},
		"invalid utf8":          {"a\xffB", "a\ufffdb"},
		"invalid utf8 word":     {"ab\xffCd", "ab_\ufffd_cd"},
	}
...
func TestKebabCase(t *testing.T) {
	cases := map[string]struct {
		...
		"empty":                 {"", ""},
		"numbers":               {"aa1", "aa1"},
		"latin accents":         {"ÉtéFoo", "été-foo"},
		"acronym then unicode":  {"HTTPÜber", "http-über"},
		"title case mid":        {"fooǅemal", "foo-ǆemal"},
		"combining mark":        {"Cafe\u0301Bar", "cafe\u0301-bar"},
		"cjk between words":     {"Foo日本Bar", "foo日本-bar"},
		"trailing underscore":   {"Été_", "été"},
	}
	for k, tc := range cases {
		t.Run(k, func(t *testing.T) {
