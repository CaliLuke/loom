...
func renderClientMain(server *Data, hasJSONRPC, hasHTTP bool) string {
	...
	writeClientAddressSetup(&b, server)
	...
	fmt.Fprintf(&b, "\t\tm, err := json.Marshal(data, json.Deterministic(true), jsontext.WithIndent(\"    \"))\n")
	fmt.Fprintf(&b, "\t\tif err != nil {\n")
	fmt.Fprintf(&b, "\t\t\tfmt.Fprintln(os.Stderr, err.Error())\n")
	fmt.Fprintf(&b, "\t\t\tos.Exit(1)\n")
	fmt.Fprintf(&b, "\t\t}\n")
...
func renderServerMain(server *Data, svcData []*service.Data, apiPkg, interPkg string, hasInterceptors bool) string {
	...
	writeServerFlagBlock(&b, server, usesHostOverrideFlags)
	fmt.Fprintf(&b, "\tflag.Parse()\n\n")

	writeServerLoggerInit(&b, server)
	...
	writeServerHostSwitch(&b, server, usesHostOverrideFlags)
...
func writeClientEndpointSelection(b *bytes.Buffer, server *Data, hasJSONRPC, hasHTTP bool) {
	...
	for _, t := range server.Transports {
		...
		writeClientTransportSelection(b, t, hasJSONRPC, hasHTTP)
	}
	fmt.Fprintf(b, "\t\tdefault:\n")
...
func writeServerHostSwitch(b *bytes.Buffer, server *Data, usesHostOverrideFlags bool) {
	...
	for _, h := range server.Hosts {
		...
		for _, u := range h.URIs {
			...
			writeServerHostURIBlock(b, h, u, usesHostOverrideFlags)
		}
	}
	fmt.Fprintf(b, "\tdefault:\n")
